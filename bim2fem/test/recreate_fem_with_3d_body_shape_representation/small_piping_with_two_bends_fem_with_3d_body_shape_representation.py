...
def main() -> int:
    ...
    ifc_arch_extruded_file_path = inlbim.api.file.write_to_ifc_spf(
        ...
        file_path=os.path.abspath(
            os.path.join(
                os.path.dirname(__file__),
                "small_piping_with_two_bends_fem_with_3d_extrusion_representation.ifc",
            )
        ),
    ...
    ifc_arch_wireframe_file_path = inlbim.api.file.write_to_ifc_spf(
        ...
        file_path=os.path.abspath(
            os.path.join(
                os.path.dirname(__file__),
                "small_piping_with_two_bends_fem_with_3d_wireframe_representation.ifc",
            )
        ),
        add_annotations=True,
    )
